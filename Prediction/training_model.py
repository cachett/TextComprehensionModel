...
"""
Neural network trying to predict eyes movement while reading: forward, backward and stable.
"""
...
for n, _ in tqdm(enumerate(train_in)):
    ...
    if output_layer == [1,0,0]:
        ...
        movement_stable.append([input_layer, output_layer])
    elif output_layer == [0,0,1]:
        movement_backward.append([input_layer, output_layer])

print(len(movement_forward), len(movement_stable), len(movement_backward))
shortest = min(len(movement_forward), len(movement_stable), len(movement_backward))

random.shuffle(movement_forward)
random.shuffle(movement_stable)
random.shuffle(movement_backward)

movement_forward = movement_forward[:shortest]
movement_stable = movement_stable[:shortest]
movement_backward = movement_backward[:shortest]

print(len(movement_forward), len(movement_stable), len(movement_backward))

all_choices = movement_forward + movement_stable + movement_backward
